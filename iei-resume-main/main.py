...
with st.sidebar:
    ...
    with st.expander('LLM & Extraction', expanded=False):
        enable_llm = st.checkbox('Enable LLM Fallback', value=False)
        force_llm = st.checkbox('Force LLM for all resumes', value=False)
        llm_provider_label = st.selectbox('LLM Provider', ['Groq (cloud)', 'Ollama (local)'], index=0)
        llm_provider = 'groq' if llm_provider_label.startswith('Groq') else 'ollama'
        ...
        fallback_to_ollama = st.checkbox('Fallback to Ollama if Groq fails', value=True)
                ...
                if enable_llm and (force_llm or row['Confidence Score'] < llm_conf_threshold):
                    try:
                        ...
                        out = call_llm(
                            ...
                            backoff_factor=float(llm_backoff),
